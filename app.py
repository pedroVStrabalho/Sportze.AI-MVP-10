...
WATER_POLO_POSITIONS = [
    "Goalkeeper",
    "Centre forward / hole set",
    "Centre back / hole D",
    "Driver / attacker",
    "Wing",
    "Point",
...
SURFING_FOCUS_OPTIONS = [
    "General surfing development",
    "Pop-up speed & take-off",
    "Bottom turn & top turn",
    "Aerial progression",
    "Paddling conditioning",
    "Balance, stance & flow",
]

SURFING_STYLE_NOTES = {
    "General surfing development": "Blend paddling, pop-up mechanics, leg strength, trunk control, then go surf and link turns.",
    "Pop-up speed & take-off": "Build explosive pop-up mechanics, shoulder stability, and fast feet before getting waves.",
    "Bottom turn & top turn": "Emphasize leg drive, rotation, balance, and then apply it while surfing.",
    "Aerial progression": "Build explosive legs, trunk stiffness, coordination, and then chase clean sections and controlled airs.",
    "Paddling conditioning": "Support surf endurance with paddling posture, upper-back strength, and controlled aerobic work.",
    "Balance, stance & flow": "Train body control, stance, compression-extension rhythm, then apply it in live surfing.",
}

BASKETBALL_FOCUS_OPTIONS = [
...
def generate_plan(
    ...
    if sport == "Baseball":
        return generate_baseball_plan(
            role, goal, level, injury_status, pain_score, session_time,
            sport_inputs["baseball_focus"], context,
            baseball_position=sport_inputs.get("baseball_position"),
        )
    if sport == "Rowing":
    ...
    if sport == "Water Polo":
        return generate_water_polo_plan(
            role, goal, level, injury_status, pain_score, session_time,
            sport_inputs["wp_focus"], context,
            wp_position=sport_inputs.get("wp_position"),
        )
    if sport == "Soccer":
        return generate_soccer_plan(
            role=role,
...
def generate_plan(
    ...
    if sport == "Soccer":
        return generate_soccer_plan(
            ...
            session_time=session_time,
            soccer_focus=sport_inputs["soccer_focus"],
            soccer_position=sport_inputs.get("soccer_position", "Mixed / multiple positions"),
            age_group=sport_inputs.get("age_group"),
            coach_level=sport_inputs.get("coach_level"),
            people_training=sport_inputs.get("people_training", 1),
            context=context,
    ...
    if sport == "Basketball":
        return generate_basketball_plan(
            role=role,
            goal=goal,
            level=level,
            injury_status=injury_status,
            pain_score=pain_score,
            session_time=session_time,
            basketball_focus=sport_inputs["basketball_focus"],
            basketball_position=sport_inputs.get("basketball_position", "Combo guard / wing"),
            context=context,
        )
    if sport == "Boxing":
    ...
    if sport == "Volleyball":
        return generate_volleyball_plan(
            ...
            context=context,
            volleyball_position=sport_inputs.get("volleyball_position"),
        )
    if sport == "Surfing":
        return generate_surfing_plan(
            role=role,
            goal=goal,
            level=level,
            injury_status=injury_status,
            pain_score=pain_score,
            session_time=session_time,
            surfing_focus=sport_inputs["surfing_focus"],
            context=context,
        )
    return "Sport not supported yet."


def generate_basketball_plan(role, goal, level, injury_status, pain_score, session_time, basketball_focus, basketball_position, context):
    if goal_is_learning(goal):
        return format_plan(
            title=f"Basketball Fundamentals Session{context['title_suffix']}",
            focus=f"Learn basketball basics — {basketball_position}",
            warmup=[
                "5 min light jog, skips, shuffle, backpedal",
                "Dynamic ankle, hip, thoracic, and shoulder mobility",
                "Ball-handling activation with both hands",
            ],
            main_work=[
                "Ball-handling series: pound dribble, crossover, in-out, retreat, 6 x 30 sec",
                "Passing block: chest, bounce, push pass, 4 rounds",
                "Finishing block: layups both sides, 4 x 5 reps",
                "Shooting form block: close-range form shooting then short jumpers",
                "Short live game or 1v1/2v2 action if partners are available",
            ],
            strength=[
                "3 x 6 goblet squats",
                "3 x 8 split squats each side",
                "3 x 6 pogo hops or low jump prep",
                "2 x 20 sec plank",
            ],
            cooldown=[
                "Walk and breathing reset",
                "Calf, quad, hip flexor, and shoulder mobility",
            ],
            coach_block=context["coach_notes"],
            notes=[
                "Keep the game stance low and balanced.",
                "Ball control and footwork come before forcing speed.",
            ],
        )

    focus_map = {
        "Ball handling & creation": [
            "Pressure dribble series with both hands",
            "Change-of-pace and change-of-direction attack reps",
            "Read-and-react cone or defender decision drill",
            "1v1 / advantage creation reps",
        ],
        "Shooting": [
            "Form shooting close to the basket",
            "Catch-and-shoot reps from key spots",
            "Off-the-dribble pull-up reps",
            "Game-speed shooting competition",
        ],
        "Finishing at the rim": [
            "Layup and stride-stop series both sides",
            "Euro step / inside hand / extension finish reps",
            "Contact finish or pad finish reps",
            "Transition finish after speed dribble",
        ],
        "Defense & footwork": [
            "Stance and slide technique block",
            "Closeout and contain reps",
            "Mirror-footwork defensive rounds",
            "Defend-then-recover transition reps",
        ],
        "Passing & decision-making": [
            "Passing on the move block",
            "Drive-and-kick decision drill",
            "Short advantage game with quick reads",
            "Transition passing into finish",
        ],
        "Transition & fast break": [
            "Outlet and lane-running reps",
            "2v1 / 3v2 transition decisions",
            "Full-court speed dribble into finish",
            "Conditioned fast-break game",
        ],
        "General basketball development": [
            "Ball-handling activation",
            "Passing and shooting block",
            "Finishing block",
            "Short live game with transition emphasis",
        ],
    }

    main = list(focus_map.get(basketball_focus, focus_map["General basketball development"]))
    if basketball_position == "Point guard":
        main.insert(0, "Ball-screen read, pace control, and decision-making reps")
    elif basketball_position in ["Shooting guard", "Small forward", "Combo guard / wing"]:
        main.insert(0, "Catch-to-shot and attack-closeout reps")
    elif basketball_position in ["Power forward", "Center"]:
        main.insert(0, "Screen, seal, rebound, and interior finishing reps")

    if role == "Coach":
        main.insert(0, "Split the team into stations to keep touches high and waiting low.")
    if beginner_modifier(level):
        main.insert(0, "Keep opposition light and prioritize technique.")
    if injury_caution(injury_status):
        main.append("Reduce jump volume and hard change-of-direction if pain is present.")

    return format_plan(
        title=f"Basketball Session{context['title_suffix']}",
        focus=f"Basketball development — {basketball_focus}" + (f" — {basketball_position}" if role == "Player" else ""),
        warmup=[
            "5-8 min movement prep: jog, shuffle, backpedal, deceleration",
            "Dynamic ankle, hip, thoracic, and shoulder mobility",
            "Ball-handling activation",
        ],
        main_work=main,
        strength=[
            "3 x 5 squats or split squats",
            "3 x 6 pogo hops or snap-down to jump",
            "2 x 8 rows or push-ups",
            "2 x 20 sec side plank",
        ],
        cooldown=[
            "Easy walk",
            "Calf, quad, adductor, hip, and thoracic mobility",
        ],
        coach_block=context["coach_notes"],
        notes=[
            "Basketball sessions should include skill, footwork, decisions, and game-speed actions.",
        ],
    )

def generate_surfing_plan(role, goal, level, injury_status, pain_score, session_time, surfing_focus, context):
    if role == "Coach":
        return format_plan(
            title=f"Surfing Coach Session{context['title_suffix']}",
            focus=f"Surf coaching session — {surfing_focus}",
            warmup=[
                "Beach mobility and movement prep",
                "Pop-up demo and foot-placement review",
                "Short paddling activation if water time is possible",
            ],
            main_work=[
                "Dry-land movement station: pop-up timing, stance, compression-extension",
                "Strength-support station: squats, split stance, trunk control",
                "Water block: wave choice, take-off timing, then theme of the day",
                "Debrief block: one cue only, then more reps",
            ],
            strength=[
            ...
            cooldown=[
                "Walk and breathing reset",
                "Hip, shoulder, thoracic, and ankle mobility",
            ],
            coach_block=context["coach_notes"],
            notes=[
                "For surf coaching, keep the land drills short and then get athletes into the water to apply them.",
            ],
        )

    focus_map = {
        "Pop-up speed & take-off": [
            "6 x 3 explosive pop-up reps with full reset",
            "4 x 20 sec fast-feet into pop-up stance",
            "4 x 5 burpee-to-surf-stance reps with clean posture",
            "Go surf and focus on early read, take-off commitment, and fast feet on the board",
        ],
        "Bottom turn & top turn": [
            "3 x 8 squats with pause in strong surf stance",
            "3 x 6 rotational med-ball or band turns each side",
            "4 x 20 sec balance and compression-extension drill",
            "Go surf and focus on setting the rail, clean bottom turns, then controlled top turns",
        ],
        "Aerial progression": [
            "4 x 5 squat jumps or box jumps",
            "3 x 6 split squats each side",
            "3 x 20 sec hollow hold / anti-rotation hold",
            "Go surf, look for the right section, and work on controlled aerial attempts and safe landings",
        ],
        "Paddling conditioning": [
            "3 x 8 band row or TRX row",
            "3 x 8 push-up plus or scap push-up",
            "4 x 30 sec prone swimmer / paddle simulation",
            "Go surf and focus on efficient paddling posture, wave count, and smart energy use",
        ],
        "Balance, stance & flow": [
            "4 x 20 sec single-leg balance each side",
            "3 x 6 lateral lunges each side",
            "3 x 6 rotational trunk control reps each side",
            "Go surf and focus on stance stability, smooth compression-extension, and linking turns",
        ],
        "General surfing development": [
            "3 x 6 goblet squats",
            "3 x 8 split squats each side",
            "3 x 8 rows or band pulls",
            "2 x 20 sec side plank each side",
            "Go surf and work on wave count, clean pop-ups, turns, and one aerial attempt if the section is right",
        ],
    }

    main = list(focus_map.get(surfing_focus, focus_map["General surfing development"]))
    if beginner_modifier(level):
        main.insert(0, "Keep all land reps technically clean and choose easier waves to apply the theme.")
    if injury_caution(injury_status):
        main.append("Reduce explosive land work and avoid painful paddling or aggressive sections.")

    return format_plan(
        title=f"Surfing Session{context['title_suffix']}",
        focus=f"Surfing development — {surfing_focus}",
        warmup=[
            "5 min easy jog or beach walk",
            "Dynamic ankle, hip, thoracic, and shoulder mobility",
            "Pop-up prep and stance activation",
        ],
        main_work=main,
        strength=[
            "Support work is already built into the main session for surfing.",
        ],
        cooldown=[
            "Walk and breathing reset after the surf",
            "Hip flexor, glute, thoracic, shoulder, and calf mobility",
        ],
        coach_block=context["coach_notes"],
        notes=[
            SURFING_STYLE_NOTES.get(surfing_focus, SURFING_STYLE_NOTES["General surfing development"]),
            "The goal is not only gym work. The session should still end by going to the water and applying it.",
        ],
    )

# Manual next-week tournament block to keep counselling stable and working.
MANUAL_NEXT_WEEK_TOURNAMENTS = [
    {
        "name": "Fayez Sarofim & Co. U.S. Men's Clay Court Championship",
        "tour": "ATP Tour",
        "level": "ATP 250",
        ...
        "estimated_direct_acceptance_best_fit": (1, 120),
        "estimated_qualifying_fit": (100, 220),
        "notes": "Official ATP Tour week of 30 Mar-5 Apr 2026.",
    },
    {
        "name": "Tiriac Open",
        "tour": "ATP Tour",
    ...
    },
    {
        ...
        "surface": "Clay",
        "start_date": date(2026, 3, 30),
        "estimated_direct_acceptance_best_fit": (90, 280),
        "estimated_qualifying_fit": (220, 450),
        "notes": "Official ATP Challenger Tour week of 30 Mar-5 Apr 2026.",
    },
    {
        "name": "Koyushokucho Miyazaki Challenger",
        "tour": "ATP Challenger Tour",
        "level": "Challenger",
        "city": "Miyazaki",
        "country": "Japan",
    ...
    },
    {
        ...
        "estimated_direct_acceptance_best_fit": (250, 900),
        "estimated_qualifying_fit": (700, 1800),
        "notes": "Official ITF week of 30 Mar-5 Apr 2026.",
        "entry_deadline": "Thu 12 Mar 2026, 14:00 GMT",
        "withdrawal_deadline": "Tue 17 Mar 2026, 14:00 GMT",
    },
    {
    ...
    },
    {
        ...
        "surface": "Hard",
        "start_date": date(2026, 3, 30),
        "estimated_direct_acceptance_best_fit": (350, 1400),
        "estimated_qualifying_fit": (900, 2200),
        "notes": "Official ITF week of 30 Mar-5 Apr 2026.",
        "entry_deadline": "Thu 12 Mar 2026, 14:00 GMT",
        "withdrawal_deadline": "Tue 17 Mar 2026, 14:00 GMT",
    },
    {
        "name": "M15 Sharm ElSheikh",
        "tour": "ITF Men's World Tennis Tour",
        "level": "ITF",
        "city": "Sharm ElSheikh",
        "country": "Egypt",
...
def fetch_live_tennis_events():
    return MANUAL_NEXT_WEEK_TOURNAMENTS

def fetch_live_tennis_events_next_week():
    return MANUAL_NEXT_WEEK_TOURNAMENTS

# Position-aware team-sport wrappers
generate_baseball_plan_base = generate_baseball_plan
def generate_baseball_plan(role, goal, level, injury_status, pain_score, session_time, baseball_focus, context, baseball_position=None):
    plan = generate_baseball_plan_base(role, goal, level, injury_status, pain_score, session_time, baseball_focus, context)
    if role == "Player" and baseball_position:
        insert = f"**Position emphasis:** {baseball_position}\n\n"
        return plan.replace("**Focus:**", insert + "**Focus:**", 1)
    return plan

generate_water_polo_plan_base = generate_water_polo_plan
def generate_water_polo_plan(role, goal, level, injury_status, pain_score, session_time, wp_focus, context, wp_position=None):
    plan = generate_water_polo_plan_base(role, goal, level, injury_status, pain_score, session_time, wp_focus, context)
    if role == "Player" and wp_position:
        insert = f"**Position emphasis:** {wp_position}\n\n"
        return plan.replace("**Focus:**", insert + "**Focus:**", 1)
    return plan

generate_volleyball_plan_base = generate_volleyball_plan
def generate_volleyball_plan(role, goal, level, injury_status, pain_score, session_time, volleyball_focus, context, volleyball_position=None):
    plan = generate_volleyball_plan_base(role, goal, level, injury_status, pain_score, session_time, volleyball_focus, context)
    if role == "Player" and volleyball_position:
        insert = f"**Position emphasis:** {volleyball_position}\n\n"
        return plan.replace("**Focus:**", insert + "**Focus:**", 1)
    return plan
# =========================
# TOP NAVIGATION
...
if st.session_state.active_section == "Training Generator":
    ...
    if sport == "Running":
        ...
        with bc1:
            ...
            if role == "Player":
                sport_inputs["baseball_position"] = st.selectbox("What position do you play?", BASEBALL_POSITIONS)
        with bc2:
            if role == "Coach":
                st.info("For coaches, baseball sessions are shaped by the team focus, not by one single position.")
            else:
                st.info("Player plans can use baseball positions to make the session more specific.")
        ...
        with wc1:
            ...
            if role == "Player":
                sport_inputs["wp_position"] = st.selectbox("What position do you play?", WATER_POLO_POSITIONS)
        with wc2:
            if role == "Coach":
                st.info("For coaches, water polo sessions stay varied and focus-led, not position-led.")
            else:
                st.info("Player plans can be adapted for goalkeeper, centre, driver, wing, point, or utility roles.")
        ...
        with sc1:
            ...
            if role == "Player":
                sport_inputs["soccer_position"] = st.selectbox("What position do you play?", SOCCER_POSITIONS)
        with sc2:
            if role == "Coach":
                sport_inputs["age_group"] = age_group
                sport_inputs["coach_level"] = coach_level
                st.info("For coaches, soccer sessions are shaped by the team focus, age band, level, player count, and game model theme.")
            else:
        ...
        with bk1:
            sport_inputs["basketball_focus"] = st.selectbox("What is the main basketball focus today?", BASKETBALL_FOCUS_OPTIONS)
            if role == "Player":
                sport_inputs["basketball_position"] = st.selectbox("What position do you play?", BASKETBALL_POSITIONS)
        with bk2:
            if role == "Coach":
                st.info("For coaches, basketball sessions are shaped by the team focus and drill theme, not one single position.")
            else:
                st.info("Player plans can be adapted for guards, wings, forwards, and centers.")
        ...
        with vc1:
            ...
            if role == "Player":
                sport_inputs["volleyball_position"] = st.selectbox("What position do you play?", VOLLEYBALL_POSITIONS)
        with vc2:
            if role == "Coach":
                st.info("For coaches, volleyball sessions are shaped by team focus and rally structure, not a single position.")
            else:
                st.info("Player plans can be adapted for setter, outside, opposite, middle, or libero roles.")

    elif sport == "Surfing":
        sf1, sf2 = st.columns(2)
        with sf1:
            sport_inputs["surfing_focus"] = st.selectbox("What is the main surfing focus today?", SURFING_FOCUS_OPTIONS)
        with sf2:
            st.info("Surfing sessions now combine dry-land work like squats, core, pop-up drills, and then live surf application.")
...
# =========================
    ...
    review_sport = st.selectbox(
        ...
        ["Tennis", "Running", "Gym / strength", "Baseball", "Rowing", "Weightlifting", "Water Polo", "Soccer", "Basketball", "Boxing", "Volleyball", "Surfing"]
    )
...
"""
            ...
            st.info(
                "This version uses a stable manual next-week tournament block so the tennis counselling keeps working reliably. "
                "You can switch back to live scraping later, but for now this avoids the crashes."
            )
